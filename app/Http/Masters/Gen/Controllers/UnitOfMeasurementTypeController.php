...
use App\Http\Masters\Gen\Services\UnitOfMeasurementTypeService;
use Symfony\Component\HttpKernel\Exception\NotFoundHttpException;
use Symfony\Component\HttpFoundation\Exception\BadRequestException;

class UnitOfMeasurementTypeController extends Controller
{
    protected $service,$unitOfMeasurementType;
    ...
    */
    ...
    /**
     * Injects UnitOfMeasurementType Service dependency through the constructor
     *
     * @param UnitOfMeasurementTypeService $service
     *
     * @return Void
     */
    public function __construct(UnitOfMeasurementTypeService $service,UnitOfMeasurementType $unitOfMeasurementType)
    {
     ...
     *
     *@link http://api/v1/gen/unit-of-measurement-type
     *
     *@method GET
     *
    ...
    /**
     * Displays a form to add new UnitOfMeasurementType
     *
     *@link http://api/v1/gen/unit-of-measurement-type/create/
     *
     *@method GET
     *
     * @return Response
    ...
    public function create()
    {
        ...
        try{
             $data = [
                'formData' => [
                    //'type' => 'Add',
                    //'title' => 'Add General Unit Of Measurement Type',
                    'fields' => $this->service->getFormFields(),
                    'formStoreUrl' => $this->baseUrl . 'gen/uom-type',
                ],
     ...
     *
     *@link http://api/v1/gen/unit-of-measurement-type/store
     *
     *@method POST
     *
     ...
     */
    public function store(UnitOfMeasurementTypeRequest $request)
    {
        $data = null;
        $code = "SUCCESS";
        try {
     ...
     /**
     * Displays a form with values to update UnitOfMeasurementType
     *
     *@link http://api/v1/gen/unit-of-measurement-type/edit/id
     *
     *@method GET
     *
     * @return Response
    ...
    public function edit(int $id)
    {
        ...
        $code = "SUCCESS";
        try {
            $tableData = $this->service->findUnitOfMeasurementTypeById($id);
            ...
            $data =  [
                ...
                // 'type' => 'Edit',
                // 'methodName' => 'PUT',
                // 'title' => 'General Unit Of Measurement Type',
                   'fields' => $data,
                   'formStoreUrl' => "{$this->baseUrl}gen/uom-type/$id",
                ]
            ];
        } catch (AuthenticationException $e) {
            $code = "HTTP_AUTHENTICATION_ERROR";
     ...
     *
     *@link http://api/v1/gen/unit-of-measurement-type/update/id
     *
     *@method PATCH/PUT
     *
     * @param UnitOfMeasurementTypeRequest  $request The request object
    ...
    */
    public function import(BulkInsertRequest $bulkInsertRequest)
    {
        ...
        try {
            ...
            $validation = [
                'name' => 'required|unique:gen_uom_type,name'
            ];
            ...
            if (!empty($diff)) {
                ...
                $import = new CustomImportsService($this->unitOfMeasurementType, $validation);
                $import->import($file);
            }
        } catch (ValidationException $e) {
            $code = "DATA_NOT_FOUND";
            $collection = (new CustomImportsService($this->unitOfMeasurementType, []))->toCollection($file);
            $failures = $e->failures();
            $fileName = $this->unitOfMeasurementType->getTable() . '_error.xlsx';
            Excel::store(
            ...
            $path = url("{$version_url}geo/country/download/{$fileName}");
            // return an error
            $bulkInsert = $this->service->getBulkInsertFormFields();
