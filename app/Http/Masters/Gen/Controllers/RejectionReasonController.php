...
use App\Http\Masters\Gen\Services\RejectionReasonService;
use Symfony\Component\HttpKernel\Exception\NotFoundHttpException;
use Symfony\Component\HttpFoundation\Exception\BadRequestException;

class RejectionReasonController extends Controller
{
    protected $service,$rejectionReason;
    ...
    */
    ...
    /**
     * Injects RejectionReason Service dependency through the constructor
     *
     * @param RejectionReasonService $service
     *
     * @return Void
     */
    public function __construct(RejectionReasonService $service,RejectionReason $rejectionReason)
    {
     ...
     *
     *@link http://api/v1/gen/rejection-reason
     *
     *@method GET
     *
    ...
    /**
     * Displays a form to add new RejectionReason
     *
     *@link http://api/v1/gen/rejection-reason/create/
     *
     *@method GET
     *
     * @return Response
    ...
    public function create()
    {
        ...
        try{
             $data = [
                'formData' => [
                    //'type' => 'Add',
                    //'title' => 'Add General Rejection Reason',
                    'fields' => $this->service->getFormFields(),
                    'formStoreUrl' => $this->baseUrl . 'gen/rejection-reason',
                ],
     ...
     *
     *@link http://api/v1/gen/rejection-reason/store
     *
     *@method POST
     *
    ...
    public function store(RejectionReasonRequest $request)
    {
     ...
     /**
     * Displays a form with values to update RejectionReason
     *
     *@link http://api/v1/gen/rejection-reason/edit/id
     *
     *@method GET
     *
     * @return Response
    ...
    public function edit(int $id)
    {
        ...
        $code = "SUCCESS";
        try {
            $tableData = $this->service->findRejectionReasonById($id);
            ...
            $data =  [
                ...
                // 'type' => 'Edit',
                // 'methodName' => 'PUT',
                // 'title' => 'General Rejection Reason',
                   'fields' => $data,
                   'formStoreUrl' => "{$this->baseUrl}gen/rejection-reason/$id",
                ]
            ];
        } catch (AuthenticationException $e) {
            $code = "HTTP_AUTHENTICATION_ERROR";
     ...
     *
     *@link http://api/v1/gen/rejection-reason/update/id
     *
     *@method PATCH/PUT
     *
     * @param RejectionReasonRequest  $request The request object
    ...
    */
    public function import(BulkInsertRequest $bulkInsertRequest)
    {
        ...
        try {
            ...
            $validation = [
                'name' => 'required|unique:gen_rejection_reason,name',
                'rejection_reason_type_id\.gen_rejection_reason_type\.name' => 'required|exists:gen_rejection_reason_type,name'
            ];
            ...
            if (!empty($diff)) {
                ...
                $import = new CustomImportsService($this->rejectionReason, $validation);
                $import->import($file);
            }
        } catch (ValidationException $e) {
            $code = "DATA_NOT_FOUND";
            $collection = (new CustomImportsService($this->rejectionReason, []))->toCollection($file);
            $failures = $e->failures();
            $fileName = $this->rejectionReason->getTable() . '_error.xlsx';
            Excel::store(
            ...
            $path = url("{$version_url}geo/city/download/{$fileName}");
            // return an error
            $bulkInsert = $this->service->getBulkInsertFormFields();
