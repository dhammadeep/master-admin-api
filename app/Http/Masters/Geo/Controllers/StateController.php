...
use App\Http\Masters\Geo\Services\StateService;
...
class StateController extends Controller
{
    ...
    */
     ...
     * @return Void
     */
    public function __construct(StateService $service, State $state)
    {
     ...
     *
     *@link http://api/v1/geo/state/create/
     *
     *@method GET
     *
     * @return Response
        ...
        try{
             $data = [
                'formData' => [
                    ...
                    'formStoreUrl' => $this->baseUrl . 'geo/state',
                ],
     ...
     *
     *@link http://api/v1/geo/state/store
     *
     *@method POST
     *
     ...
     *
     *@link http://api/v1/geo/state/edit/id
     *
     *@method GET
     *
     * @return Response
    ...
    public function edit(int $id)
    {
        ...
        $code = "SUCCESS";
        try {
            $tableData = $this->service->findStateById($id);
            $data = $this->service->getFormFields();
            ...
            $data =  [
                'formData' => [
                   'fields' => $data,
                   'formStoreUrl' => "{$this->baseUrl}geo/state/$id",
                ]
            ];
        } catch (AuthenticationException $e) {
            $code = "HTTP_AUTHENTICATION_ERROR";
     ...
     *
     *@link http://api/v1/geo/state/update/id
     *
     *@method PATCH/PUT
     *
    ...
    */
    public function import(BulkInsertRequest $bulkInsertRequest)
    {
        ...
        try {
            ...
            $validation = [
                'name' => 'required|unique:geo_state,name',
                'country_id\.geo_country\.name' => 'required|exists:geo_country,name'
            ];
            ...
            if (!empty($diff)) {
                ...
                $import = new CustomImportsService($this->state, $validation);
                $import->import($file);
            }
        } catch (ValidationException $e) {
            $code = "DATA_NOT_FOUND";
            $collection = (new CustomImportsService($this->state, []))->toCollection($file);
            $failures = $e->failures();
            $fileName = $this->state->getTable() . '_error.xlsx';
            Excel::store(
