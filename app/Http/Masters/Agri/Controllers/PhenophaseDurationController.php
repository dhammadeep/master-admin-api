...
use App\Http\Masters\Agri\Services\PhenophaseDurationService;
use Symfony\Component\HttpKernel\Exception\NotFoundHttpException;
use Symfony\Component\HttpFoundation\Exception\BadRequestException;

class PhenophaseDurationController extends Controller
{
    protected $service,$phenophaseDuration;
    ...
    */
     ...
     * @return Void
     */
    public function __construct(PhenophaseDurationService $service,PhenophaseDuration $phenophaseDuration)
    {
        $this->service = $service;
        $this->phenophaseDuration = $phenophaseDuration;
    }

    /**
     * List page to display all data with pagination support.
     *
     *@link http://api/v1/agri/phenophase-duration
     *
     *@method GET
     *
     ...
     *
     *@link http://api/v1/agri/phenophase-duration/create/
     *
     *@method GET
     *
     * @return Response
    ...
    public function create()
    {
        ...
        try{
             $data = [
                'formData' => [
                    //'type' => 'Add',
                    //'title' => 'Add Agriculture Phenophase Duration',
                    'fields' => $this->service->getFormFields(),
                    'formStoreUrl' => $this->baseUrl . 'agri/phenophase-duration',
                ],
     ...
     *
     *@link http://api/v1/agri/phenophase-duration/store
     *
     *@method POST
     *
     ...
     *
     *@link http://api/v1/agri/phenophase-duration/edit/id
     *
     *@method GET
     *
     * @return Response
    ...
    public function edit(int $id)
    {
        ...
        $code = "SUCCESS";
        try {
            $tableData = $this->service->findPhenophaseDurationById($id);
            ...
            $data =  [
                ...
                // 'type' => 'Edit',
                // 'methodName' => 'PUT',
                // 'title' => 'Agriculture Phenophase Duration',
                   'fields' => $data,
                   'formStoreUrl' => "{$this->baseUrl}agri/phenophase-duration/$id",
                ]
            ];
        } catch (AuthenticationException $e) {
            $code = "HTTP_AUTHENTICATION_ERROR";
     ...
     *
     *@link http://api/v1/agri/phenophase-duration/update/id
     *
     *@method PATCH/PUT
     *
    ...
    public function import(BulkInsertRequest $bulkInsertRequest)
    {
        ...
        try {
            ...
            $validation = [
                ...
                'variety_id\.agri_variety\.name' => 'required|exists:agri_variety,name',
                'start_das' => 'required',
                'end_das' => 'required'
            ];
            ...
            if (!empty($diff)) {
                ...
                $import = new CustomImportsService($this->phenophaseDuration, $validation);
                $import->import($file);
            }
        } catch (ValidationException $e) {
            $code = "DATA_NOT_FOUND";
            $collection = (new CustomImportsService($this->phenophaseDuration, []))->toCollection($file);
            $failures = $e->failures();
            $fileName = $this->phenophaseDuration->getTable() . '_error.xlsx';
            Excel::store(
